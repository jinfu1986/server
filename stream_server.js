...
(function() {
  var Bits, CustomReceiver, DEBUG_INCOMING_PACKET_DATA, DEBUG_INCOMING_PACKET_HASH, DEFAULT_SERVER_NAME, Sequent, StreamServer, aac, avstreams, config, crypto, fs, path, h264, http, logger, mp4, net, packageJson, ref, rtmp, rtsp, serverName;
  ...
  StreamServer = (function() {
    ...
    StreamServer.prototype.dumpToFile = function(streamId) {
      ...
      db.collection('streams').updateOne({ key: streamId, islive: true }, { $set: { thumb_url: 'http://' + config.serverAddress + ':' + config.serverPort + '/' + screenshot_file_name, stream_url: `rtmp://${serverAddr}/`+streamId }});
      ...
      var screenshot_file_name = 'file/screenshots/'+startTime.getTime()+'-'+dumpId+'.png';
      var screenshotProc = spawn('ffmpeg', [ '-i', `rtmp://${serverAddr}/`+streamId, '-f', 'image2', '-vframes', '1', 'public/' + screenshot_file_name]);

      setTimeout(function() {
        db.collection('streams').findOneAndUpdate({ key: dumpId, islive: true }, { $set: { thumb_url: screenshot_file_name, stream_url: streamId }}, { returnNewDocument: true });
      }, 1000);

      //var fileName = dumpId + '_' + uuid.v1() + '.flv';
      var fileName = uuid.v1() + '.flv';
      ...
      dumpProc.on('close', function() {
        var endTime = new Date(new Date().getTime() - 1000*30);
        console.log(`Stream dump is finished. File could be found at file/${fileName}`);
        var p = __dirname + '/public/file/';
        var newFileName = startTime.getTime() + '-' + endTime.getTime() + '-' + dumpId + '.mp4'; // 30s delay on process complete

        var convertCmd = 'ffmpeg';
        var convertArgs = [
          '-i', path.join(p, fileName),
          path.join(p, newFileName)
        ];
        var convertProc = spawn(convertCmd, convertArgs);
        ...
        convertProc.on('close', function() {
          fs.unlink(path.join(p, fileName), function(err) {
            if(err) throw err;
            console.log(`Status: ${fileName} converted to ${newFileName}`);
            console.log(`Status: ${fileName} removed.`);
            ...
            db.collection('streams').findOneAndUpdate({
              ...
              db.collection('users').findOneAndUpdate({
                _id: ObjectID(doc.user_id.toString()),
              }, {
                $push: { videos: doc._id }
              })
            });
            // .then((doc) => {
            //   var streamEntry = doc;
            //   db.collection('videos').save({
            //     thumb_url: streamEntry.thumb_url,
            //     video_url: 'file/' + newFileName,
            //     event_name: streamEntry.title,
            //     key: streamEntry.key,
            //     user_id: '',
            //     length: duration,
            //     started: dateParseShort(startTime),
        ...
        // fs.rename(path.join(p, fileName), path.join(p, newFileName), function (err) {
        //   if(err) throw err;
        //   console.log(`Status: ${fileName} moved to ${newFileName}`);
        // });
      });
    };
